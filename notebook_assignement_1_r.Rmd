...
Calculate the ink sum again, but this time its on the images that represent detected straight lines on the mnist images, so more lines = straighter object less lines = rounder object. Which means that the amount of ink will determine how straight or curved the digit is. This, combined with the original ink sum feature of the images will make differentiating between them easier.
```{r}
...
```{r}
straightness_sd <- aggregate(straight_line_df$ink_sum,by=list(straight_line_df$y),FUN=sd )
straightness_mean <- aggregate(straight_line_df$ink_sum,by=list(straight_line_df$y),FUN=mean )
