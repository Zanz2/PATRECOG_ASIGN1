...
if(!(file.exists("line_image_matrix.rds"))){
  ...
  line_image_matrix <- readRDS("line_image_matrix.rds") # skip the below and just read it
  print("done")
}
```
...
table(new_df$label,multinom.pred)
```
The table is a lot better than using only the ink sum feature, and somewhat better than using the ink sum feature with the standard deviation for pixels inside each image, in most cases, out of all the predictions for a certain image, the correct class had the most counts out of all other classes (0,1,2,3,5,7,8,9)
4 was getting confused with 5, the straight line detector produces a simillar amount of straight edges for both usually.
6 was getting confused with 8, because they produce simillar straight line detection images, they also look simillar.


Overall the results are considerably better than our first prediction, with the predictor still confusing 4's with 5's and 6's with 8's.
...
```{r}
library(glmnet)
# removes all the columns with labels ink used  etc

#useless_pixels <- get_useless_indexes(mnist.dat)

test_set <- mnist.dat[5001:nrow(mnist.dat),-c(ncol(mnist.dat),3,2,1)]
train_set <- mnist.dat[1:5000,-c(ncol(mnist.dat),3,2,1)]

#Create 10 equally size folds
folds <- cut(seq(1,nrow(train_set)),breaks=10,labels=FALSE)
...
for(i in 1:10){
    ...
    lasso <- cv.glmnet(as.matrix(trainData),trainData[,1]
